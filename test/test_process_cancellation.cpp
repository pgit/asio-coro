...
class ProcessCancellation : public ProcessBase, public testing::Test
{
protected:
...
//
TEST_F(ProcessCancellation,
       WHEN_throw_if_cancelled_is_set_to_false_THEN_does_not_throw_on_next_await)
{
   test = [this](readable_pipe out, bp::process child) -> awaitable<ExitCode>
   {
      auto [ec] = co_await co_spawn(executor, log_stdout(out), as_tuple);
