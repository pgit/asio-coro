...
class ProcessExecute : public ProcessBase, public testing::Test
{
protected:
...
TEST_F(ProcessExecute, WHEN_ping_is_started_THEN_completes_gracefully)
{
...
TEST_F(ProcessExecute, WHEN_multiple_pings_are_started_THEN_all_of_them_complete_gracefully)
{
   ...
   EXPECT_CALL(*this, on_stdout(_)).Times(AtLeast(1));
   EXPECT_CALL(*this, on_stdout(HasSubstr("rtt"))).Times(N);
...
//
TEST_F(ProcessExecute, DISABLED_Top)
{
...
TEST_F(ProcessExecute, WHEN_ping_is_cancelled_terminal_THEN_is_terminated)
{
   co_spawn(executor, execute("/usr/bin/ping", {"::1", "-c", "5", "-i", "0.1"}),
            cancel_after(250ms, cancellation_type::terminal, token()));

   EXPECT_CALL(*this, on_stdout(_)).Times(AtLeast(1));
   EXPECT_CALL(*this, on_stdout(HasSubstr("PING")));
   EXPECT_CALL(*this, on_stdout(HasSubstr("rtt"))).Times(0);
   EXPECT_CALL(*this, on_exit(SIGKILL)); // 9
}

TEST_F(ProcessExecute, WHEN_ping_is_cancelled_partial_THEN_exists_with_code_15)
{
   co_spawn(executor, execute("/usr/bin/ping", {"::1", "-c", "5", "-i", "0.1"}),
...
TEST_F(ProcessExecute, WHEN_ping_is_cancelled_total_THEN_exists_gracefully)
{
   co_spawn(executor, execute("/usr/bin/ping", {"::1", "-c", "5", "-i", "0.1"}),
...
//
TEST_F(ProcessExecute, WHEN_custom_cancellation_slot_is_emitted_THEN_process_is_cancelled)
{
...
TEST_F(ProcessExecute, WHEN_sleep_is_cancelled_terminal_THEN_exits_with_code_9)
{
   co_spawn(executor, execute("/usr/bin/sleep", {"10"}), cancel_after(50ms, token()));
   EXPECT_CALL(*this, on_exit(SIGKILL));
}

TEST_F(ProcessExecute, WHEN_sleep_is_cancelled_partial_THEN_exits_with_code_15)
{
...
TEST_F(ProcessExecute, WHEN_sleep_is_cancelled_total_THEN_exits_with_code_2)
{
   co_spawn(executor, execute("/usr/bin/sleep", {"10"}),
...
TEST_F(ProcessExecute, WHEN_cancelled_terminal_THEN_exits_with_code_9)
{
   ...
   EXPECT_CALL(*this, on_stdout(_)).Times(AtLeast(1));
   EXPECT_CALL(*this, on_stdout(HasSubstr("done"))).Times(0);
   EXPECT_CALL(*this, on_exit(SIGKILL));
}
...
TEST_F(ProcessExecute, WHEN_cancelled_partial_THEN_receives_sigterm_and_exits_gracefully)
{
   ...
   EXPECT_CALL(*this, on_stdout(_)).Times(AtLeast(1));
   EXPECT_CALL(*this, on_stdout(HasSubstr("SIGTERM"))).Times(1);
   EXPECT_CALL(*this, on_stdout(HasSubstr("done"))).Times(1);
   EXPECT_CALL(*this, on_exit(0));
}
...
TEST_F(ProcessExecute, WHEN_cancelled_total_THEN_receives_sigint_and_exits_gracefully)
{
   ...
   EXPECT_CALL(*this, on_stdout(_)).Times(AtLeast(1));
   EXPECT_CALL(*this, on_stdout(HasSubstr("SIGINT"))).Times(1);
   EXPECT_CALL(*this, on_stdout(HasSubstr("done"))).Times(1);
   EXPECT_CALL(*this, on_exit(0));
}
...
//
TEST_F(ProcessExecute, DISABLED_WHEN_bash_is_killed_THEN_exits_with_code_9)
{
   co_spawn(executor,
...
TEST_F(ProcessExecute, WHEN_bash_ignores_sigterm_THEN_sleep_finishes)
{
   co_spawn(executor,
...
TEST_F(ProcessExecute, WHEN_bash_ignores_sigint_THEN_sleep_finishes)
{
   co_spawn(executor,
...
TEST_F(ProcessExecute, WHEN_no_newline_at_end_of_output_THEN_prints_line)
{
   co_spawn(executor, execute("/usr/bin/echo", {"-n", "No newline at the end of this"}), token());
   EXPECT_CALL(*this, on_stdout("No newline at the end of this"));
   EXPECT_CALL(*this, on_exit(0));
}

TEST_F(ProcessExecute, WHEN_multiple_lines_are_in_last_buffer_THEN_all_are_print)
{
   co_spawn(executor, execute("/usr/bin/echo", {"-ne", "First line\nLast line"}), token());
   EXPECT_CALL(*this, on_stdout("First line"));
   EXPECT_CALL(*this, on_stdout("Last line"));
   EXPECT_CALL(*this, on_exit(0));
}

TEST_F(ProcessExecute, WHEN_process_succeeds_THEN_returns_zero_exit_code)
{
   co_spawn(executor, execute("/usr/bin/true", {}), token());
   EXPECT_CALL(*this, on_exit(0));
}

TEST_F(ProcessExecute, WHEN_process_fails_THEN_returns_non_zero_exit_code)
{
   co_spawn(executor, execute("/usr/bin/false", {}), token());
   EXPECT_CALL(*this, on_exit(Not(0)));
}

TEST_F(ProcessExecute, WHEN_path_does_not_exist_THEN_raises_no_such_file_or_directory)
{
