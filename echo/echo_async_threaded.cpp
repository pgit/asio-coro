...
private:
   void do_read()
   {
      ...
      socket_.async_read_some(buffer(data_), [this, self](error_code ec, std::size_t length)
      {
         if (!ec)
            do_write(length);
      });
...
{
public:
   explicit server(tcp::acceptor acceptor) : acceptor_(std::move(acceptor)) { do_accept(); }
