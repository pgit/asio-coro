...
awaitable<void> server(tcp::acceptor acceptor)
{
   ...
   auto slot = (co_await this_coro::cancellation_state).slot();
...
awaitable<void> server_interruptable()
{
   auto executor = co_await this_coro::executor;
   signal_set signals(executor, SIGINT, SIGTERM);
   co_await (server({executor, {tcp::v6(), 55555}}) || signals.async_wait(use_awaitable));
}

int main()
{
   io_context context;
   co_spawn(context, server_interruptable(), detached);
   context.run();
}
